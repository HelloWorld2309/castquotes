...
/**
 * CastQuotes with small local Admin panel
 * - Shows one random quote per visit
 * - "Cast this quote" button with clipboard/postMessage fallback
 * - Small admin panel (⚙) to add/delete quotes, protected by a local password
 *
 * How to open admin:
 * - Click the small ⚙ icon top-right
 * - Default password: basedfin
 * - You can change the password inside the admin (saved to localStorage)
 */
...
export default function App(): JSX.Element {
  ...
  useEffect(() => {
    // pick one quote once per visit (on mount)
    setQuote(pickRandomQuote(quotes));
    // keep adminPw in sync with storage in case user changed via another tab
    const pw = getAdminPw();
    setAdminPwState(pw);
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  // When quotes change, persist and keep current quote unchanged for the visit
  useEffect(() => {
    saveQuotes(quotes);
  }, [quotes]);

  const tryOpenFarcasterComposer = async (text: string) => {
    // 1) Try global SDK object
    try {
      // @ts-ignore
    ...
    // 2) postMessage to parent (host may listen)
    try {
  ...
  const handleSetPw = () => {
    ...
    try {
      ...
      setAdminPw(t);
      setAdminPwState(t);
      localStorage.setItem(ADMIN_PW_KEY, t);
            ...
            {authStep === "locked" ? (
              <div>
                ...
                <div style={{ marginTop: 8 }}>
                  <button onClick={tryUnlock} style={styles.smallBtn}>
                    Unlock
                  </button>{" "}
                  <button
                    onClick={() => {
                      // quick hint: default password shown for convenience in dev
                      alert("Default password (local-only): basedfin");
                    }}
              ...
              <div>
                ...
                <div style={{ marginBottom: 10 }}>
                  <input
                    ...
                    placeholder="Add new quote here..."
                    style={styles.input}
                  />
                  <button onClick={handleAddQuote} style={{ ...styles.smallBtn, marginLeft: 8 }}>
                    Add
                  </button>
                </div>
                ...
                <div style={{ maxHeight: 260, overflow: "auto", borderRadius: 8, padding: 6 }}>
                  {quotes.map((q, i) => (
                    <div key={i} style={styles.quoteRow}>
                      ...
                      <button onClick={() => handleDeleteQuote(i)} style={styles.delBtn}>
                        Delete
                      </button>
                    </div>
                  ))}
                ...
                <div style={{ marginTop: 12 }}>
                  ...
                  <input
                    ...
                    style={styles.input}
                  />
                  <button onClick={handleSetPw} style={{ ...styles.smallBtn, marginLeft: 8 }}>
                    Save password
                  </button>
                </div>

                <div style={{ marginTop: 12 }}>
                  <button onClick={handleResetToDefault} style={styles.warnBtn}>
